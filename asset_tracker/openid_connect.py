...
class OpenIDConnectClient(object):
    ...
    def request_user_authentication(self, url, client_redirect_endpoint, csrf_token, user_target_path):
        ...
        rta_authorization_request = self.oauth_client.prepare_request_uri(url, scope=['openid'], redirect_uri=client_redirect_endpoint, state=encoded_state)
    ...
    def set_cookies(self, response, access_token, refresh_token, expires_in):
        ...
        response.set_cookie('rta_rt', signed_serialize(refresh_token, self.cookie_signature), max_age=604800)
    ...
    def verify_authorization_response(self, url, session_csrf_token):
        ...
        if csrf_token == session_csrf_token:
            return authorization_code, user_target_path
        else:
            raise HTTPBadRequest()
